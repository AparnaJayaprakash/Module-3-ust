...
//	Hibernate: insert into products (description, name, price, qty, product_id) values (?, ?, ?, ?, ?)
	...
	@GeneratedValue(strategy = GenerationType.IDENTITY)   //for this strategy to work first we have to create a table in mysql database and explicitly specify the auto-increment(AI) column as checked for the primary key column.
